...
backend to `127.0.0.1:8081` and the frontend dev server runs on `127.0.0.1:8080`.
...
- **Local exposure guards** — `apps/api/src/realmock/platform/core/local_only.py`:
  `require_local_peer` restricts management endpoints (profile / resume /
  settings / interview / prep) to loopback peers; in `ENV=prod` the `TEST_MODE`
...
- **Secrets at rest** — `apps/api/src/realmock/platform/core/secrets.py`:
  ...
  `data/.secret.key`). `security/redact.py` redacts API-key-shaped strings
  from logs.
