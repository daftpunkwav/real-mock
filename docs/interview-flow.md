...
| 5 | records-domain `agents/report/agent.py: DeepReportAgent` — two-stage ReAct pipeline over the frozen ledger: stage 1 turn notes (12 turns per batch, up to 2 batches in parallel), stage 2 synthesis producing the aligned verdict, scores, highlights / key problems and the training plan; wall-clock budget 480 s |
