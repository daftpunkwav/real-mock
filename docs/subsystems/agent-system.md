...
| `executor.py` | `invoke_with_timeout()` — single-call timeout + error classification; timeouts and unexpected exceptions return as JSON observations (`"timeout"` / `"tool_failed"`), `ApiBusinessError` raises. Also `ToolRunGuard`: the per-loop policy (total-call budget + same-args circuit breaker) shared by tool-calling agents. |
...
- **Growth insight agent** — [`domains/growth/agents/insight.py`](../../apps/api/src/realmock/domains/growth/agents/insight.py): cross-session analysis over a bounded tool loop (history / resume / profile evidence, same guard semantics as the resume-review loop); the payload persists in `growth_insights` and regenerates single-flight via `insight_scheduler` after each finished interview.
