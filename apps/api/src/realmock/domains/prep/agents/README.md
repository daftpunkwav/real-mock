...
| `persist.py` | Turn persistence: compaction events, message finalization, usage deltas with request diagnostics |
| `memory_precipitate.py` | End-of-turn curation: one advisory LLM call decides whether the turn is worth a long-term memory; writes at most one, never raises |
