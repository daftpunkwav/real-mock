...
| 模块 | 用途 |
| --- | --- |
| `chat.py` | 轮次编排:同步单轮与事件流式回答;ask_user 对话在两条通道均有承载 |
...
| `persist.py` | 轮次持久化:压缩事件、消息定稿、用量增量与请求诊断;失败的 turn 仍会保存用户问题,损坏历史在覆盖前先备份 |
| `memory_precipitate.py` | 轮末记忆沉淀:一次咨询式 LLM 调用判定本 turn 是否值得存入长期记忆;至多写一条,绝不抛错。以 detached 任务在轮次结束后运行,不阻塞响应且不受客户端断连影响 |
