...
| Package | Purpose |
| --- | --- |
| `capabilities/` | External capability adapters: `ai/` (LLM providers, agent loop, context management), `integrations/github/`, `knowledge/search/`, `voice/` (STT / TTS / voice config) |
...
| `core/` | Kernel utilities: constants, errors and handlers, logging, file locks, local-only guard, DB migration helpers, agent error log |
