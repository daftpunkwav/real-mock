...
export function AnalyzeStageProgress({ progress }: { progress?: ReviewLiveState }) {
  ...
  return (
    <div className="px-5 py-5 sm:px-7 sm:py-6">
      <div className="grid min-h-[22rem] grid-cols-1 gap-5 md:h-[min(70vh,36rem)] md:grid-cols-[minmax(13rem,17rem)_minmax(0,1fr)] md:gap-0">
        <section
          aria-label={t("stage.plan")}
          className="md:min-h-0 md:h-full md:overflow-y-auto md:border-r md:border-surface-border md:pr-6"
        >
        ...
        <section aria-label={t("stage.log")} className="flex min-h-0 flex-col md:h-full md:pl-6">
          <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-subtle">
            {t("stage.log")}
          </p>
          <div
